    ...
    let handler = Arc::new(move |args: Value, _ctx: ToolExecutionContext| {
        ...
        if let Some(expected) = args.get("conversation_id").and_then(|v| v.as_str()) {
            if let Some(actual) = record.conversation_id.as_ref() {
                if actual != expected {
                    return Err(ToolError::new("conversation_id does not match stored record"));
                }
            } else {
        ...
        serde_json::to_value(record).map_err(|err| ToolError::new(format!(
            "Failed to serialize tool output record: {err}"
        )))
    });
